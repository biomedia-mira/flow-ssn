...
def prepare_data(input_file, output_file):
    ...
    for key, value in data.items():
        ...
        hash_id = hash(s_id)  # Checked manually that there are no collisions
        # hash_id = hashlib.md5(s_id.encode("utf-8")).hexdigest()  # reproducible
        uids[tt].append(hash_id)

    for tt in ["test", "train", "val"]:

        groups[tt].create_dataset("uids", data=np.array(uids[tt], dtype=np.int64))
        groups[tt].create_dataset("labels", data=np.array(labels[tt]).astype(np.uint8))
        groups[tt].create_dataset(
            "images", data=np.array(images[tt]).astype(np.float32)
        )
...
if __name__ == "__main__":
    ...
    d = load_and_maybe_process_data(
        input_file, preprocessing_folder, force_overwrite=True
    )
